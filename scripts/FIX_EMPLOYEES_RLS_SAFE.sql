-- =====================================================
-- SCRIPT PARA ARREGLAR POLÍTICAS RLS DE EMPLOYEES (VERSIÓN SEGURA)
-- =====================================================

-- PASO 1: Eliminar TODAS las políticas existentes de employees
...
DROP POLICY IF EXISTS "Enable read access for company employees" ON employees;
DROP POLICY IF EXISTS "Enable insert for company employees" ON employees;
DROP POLICY IF EXISTS "Enable update for company employees" ON employees;
DROP POLICY IF EXISTS "Enable delete for company employees" ON employees;
...
-- Asegurar que RLS esté habilitado en la tabla employees
ALTER TABLE employees ENABLE ROW LEVEL SECURITY;

-- =====================================================
-- PASO 6: VERIFICAR FUNCIONES CREADAS
-- =====================================================

-- Verificar que las funciones existen
DO $$
BEGIN
  -- Verificar check_cedula_unique
  IF EXISTS (
    SELECT 1 FROM pg_proc WHERE proname = 'check_cedula_unique'
  ) THEN
    RAISE NOTICE '✅ Función check_cedula_unique creada correctamente';
  ELSE
    RAISE WARNING '❌ Función check_cedula_unique NO fue creada';
  END IF;
  
  -- Verificar create_employee_for_company  
  IF EXISTS (
    SELECT 1 FROM pg_proc WHERE proname = 'create_employee_for_company'
  ) THEN
    RAISE NOTICE '✅ Función create_employee_for_company creada correctamente';
  ELSE
    RAISE WARNING '❌ Función create_employee_for_company NO fue creada';
  END IF;
END $$;
...
BEGIN
  ...
  IF policy_count >= 4 THEN
    RAISE NOTICE '✅ Políticas RLS de employees creadas correctamente (% políticas)', policy_count;
  ELSE
    RAISE WARNING '❌ Solo % políticas RLS encontradas para employees (se esperaban 4)', policy_count;
  END IF;
END $$;
...
BEGIN
  ...
  RAISE NOTICE '1. Prueba crear un empleado desde la interfaz';
  RAISE NOTICE '2. Verifica que no aparezcan errores 404 o 403';
  RAISE NOTICE '3. Prueba descargar desprendibles desde el sistema';
END $$;